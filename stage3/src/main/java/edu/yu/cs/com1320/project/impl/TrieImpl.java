...
public class TrieImpl<Value> implements Trie<Value> {
    ...
    public Set<Value> deleteAllWithPrefix(String prefix) {
        ...
        //get the prefix node so we can save all values in teh subtrie.
        Node prefixNode = this.get(this.root, prefix, 0);
        if (prefixNode == null) {
        ...
        this.root = this.deleteAllWithPrefix(this.root, prefix, 0, new HashSet<Value>());
        return deletedValues;
    }

    private Node deleteAllWithPrefix(Node<Value> currentNode, String prefix, int index, Set<Value> deletedSet) {
        if (currentNode == null) {
            return null;
        }
        ...
        //otherwise, continue down the trie to the prefix node
            ...
            currentNode.links[i] = this.deleteAllWithPrefix(currentNode.links[i], prefix, index + 1, deletedSet);
        }
